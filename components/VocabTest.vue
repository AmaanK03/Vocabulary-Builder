<template>
  <div>
    ...
    <form action="#" @submit.prevent="onSubmit">
      ...
      <!-- I add the Urdu input field so users can type urdu words in the text field -->
      <div class="ui labeled input fluid">
        <div class="ui label">
          <i class="pakistan flag"></i> 
          Urdu
        </div>
        <input type="text" readonly :disabled="testOver" :value="currWord.urdu"/>
      </div>
      ...
      <div class="ui labeled input fluid">
        ...
        <input type="text" placeholder="Enter English translation..." v-model="english" :disabled="testOver" autocomplete="off" />
      </div>
  ...
  methods: {
    onSubmit: function() {
      if (this.english === this.currWord.english) {
        ...
        this.incorrectGuesses.push(`${this.currWord.german} (Urdu: ${this.currWord.urdu})`);
      }
